...
export const editCourse = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Check if thumbnail is present and is a valid string
      if (data.thumbnail && typeof data.thumbnail === "string") {
        if (!data.thumbnail.startsWith("https")) {
          ...
          data.thumbnail = {
            public_id: myCloud.public_id, // Ensure public_id is provided
            url: myCloud.secure_url,
          };
        } else {
          // Get the public_id from the existing thumbnail
          const public_id = courseData.thumbnail
            ? courseData.thumbnail.public_id
            : null;
...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // Kiểm tra xem người dùng có trong danh sách khóa học đã mua không
